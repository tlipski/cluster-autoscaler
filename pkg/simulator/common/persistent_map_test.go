...
// TestStoreOverwritesNonComparableValue covers the value types the simulator actually
// stores, several of which - []*ResourceSlice among them - cannot be compared with ==.
// Overwriting must not depend on being able to tell the old value from the new one.
func TestStoreOverwritesNonComparableValue(t *testing.T) {
	m := newPersistentMap[string, []int]()
	m = m.Store("key", []int{1, 2})
	m = m.Store("key", []int{3})

	value, ok := m.Load("key")
	if !ok {
		t.Fatal("Load() after overwrite: key missing")
	}
	if len(value) != 1 || value[0] != 3 {
		t.Errorf("Load() after overwrite = %v, want [3]", value)
	}
	if m.Len() != 1 {
		t.Errorf("Len() after overwriting the same key = %d, want 1", m.Len())
	}

	// Storing a value equal to the one already there is still an overwrite, not a
	// no-op, and must leave the map readable.
	same := []int{3}
	m = m.Store("key", same)
	if value, ok := m.Load("key"); !ok || len(value) != 1 || value[0] != 3 {
		t.Errorf("Load() after storing an equal value = %v, %t, want [3], true", value, ok)
	}
}
