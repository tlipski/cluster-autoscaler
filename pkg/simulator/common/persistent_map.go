...
// hashSeed is process-wide, so hashes are only ever compared between maps built
// in the same process. Randomising it per process is what keeps the trie shape
// from being predictable from the keys alone.
var hashSeed = maphash.MakeSeed()

// hashKey hashes an arbitrary comparable key. maphash.Comparable applies the
// runtime's own hasher for K without boxing the key into an interface, so struct
// keys such as ResourceClaimId cost no allocation to hash.
//
// Only the low 32 bits are used: the trie consumes the hash 5 bits at a time and
// bottoms out in a collision node after 7 levels, so the remaining bits would
// never be looked at.
...
func (n *bitmapIndexedNode[K, V]) assoc(session *editSession, shift uint, hash uint32, key K, val V, addedLeaf *box) node[K, V] {
	...
	if (n.bitmap & bit) != 0 {
		...
		if entry.key == key {
			// Overwriting a key with an equal value could return n untouched, but
			// establishing equality needs V to be comparable, which it is not. Inside
			// an edit session ensureEditable hands back n anyway, and the PatchSet only
			// ever writes through a session, so the copy this would save is not one the
			// simulator pays.
			editable := n.ensureEditable(session)
