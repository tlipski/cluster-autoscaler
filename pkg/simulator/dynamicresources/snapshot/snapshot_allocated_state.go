...
// reset drops all tracked state, so that it gets rebuilt on the next read.
func (t *allocatedStateTracker) reset() {
	t.devices = newRefCountedSet[structured.DeviceID]()
	t.allDevices = newRefCountedSet[structured.DeviceID]()
	t.sharedDeviceIDs = newRefCountedSet[structured.SharedDeviceID]()
	t.capacity = structured.NewConsumedCapacityCollection()
	t.contributions = map[ResourceClaimId]*claimContribution{}
	t.built = false
}
