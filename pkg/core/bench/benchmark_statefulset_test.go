		...
		b.Run(name, func(b *testing.B) {
			s := scenario{
				...
				config: func(opts *config.AutoscalingOptions) {
					...
					// defaultCAOptions builds the struct directly rather than
					// through flag parsing, so this lands on the zero value even
					// though the flag defaults to true. Left false, the CSI
					// snapshot is never populated and the CSINodes this scenario
					// creates are inert - which is most of what makes a
					// PVC-backed workload different from a plain one.
					opts.CSINodeAwareSchedulingEnabled = true
				},
			}
			s.run(b)
		})
